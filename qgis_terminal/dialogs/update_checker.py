"""
Update Checker Dialog for QGIS Terminal
...
GITHUB_REPO = "opengeos/qgis-terminal-plugin"
GITHUB_BRANCH = "main"
PLUGIN_PATH = "qgis_terminal"
...
def _require_https(url: str) -> str:
    ...
    Guards against accidental file:// or http:// schemes reaching urlopen
    or urlretrieve. The plugin only ever fetches from a hard-coded GitHub
    URL, so any other scheme indicates a programming error.

    Args:
        url: The URL to validate.

    Returns:
        The URL unchanged if it uses https.
...
class VersionCheckWorker(QThread):
    ...
    def run(self):
        ...
        try:
            # URL is the hard-coded https GitHub raw endpoint above; the
            # _require_https guard rejects any non-https scheme.
            with urlopen(
...
class DownloadWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            temp_dir = tempfile.mkdtemp(prefix="qgis_terminal_update_")
            ...
            # URL is the hard-coded https GitHub archive endpoint above; the
            # _require_https guard rejects any non-https scheme.
            ...
            # The structure will be: repo-main/qgis_terminal/
            extracted_plugin_dir = None
            for root, dirs, files in os.walk(extract_dir):
                if "metadata.txt" in files and "qgis_terminal.py" in files:
            ...
            try:
                ...
                def is_valid_plugin_dir(path):
                    ...
                    ) and os.path.exists(os.path.join(path, "qgis_terminal.py"))
...
class UpdateCheckerDialog(QDialog):
    ...
    def __init__(self, plugin_dir, parent=None):
        ...
        self.setWindowTitle("QGIS Terminal Update")
    ...
    def _setup_ui(self):
        ...
        # Header
        header_label = QLabel("QGIS Terminal Update Checker")
        header_font = QFont()
    ...
    def _on_download_finished(self, install_path):
        ...
        QMessageBox.information(
            self,
            "Update Complete",
            f"QGIS Terminal has been updated to version {self.latest_version}.\n\n"
